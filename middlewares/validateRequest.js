...
const validateRequest = (req, res, next, schema, isQuery = false, isParam = false) => {
  const options = {
  ...
  if (error) {
    ...
    setSuccessResponse(res, StatusCodes.NOT_ACCEPTABLE, false, customErrorObj, "validation error");
  } else {
    req.body = value;
    next();
  }
